      ...
      {client && (
        <>
          <dl className="mt-4 grid grid-cols-1 gap-x-6 gap-y-3 border-t border-slate-100 pt-4 sm:grid-cols-2">
            ...
            <Field label="Edad" value={client.birth_date ? `${calculateAge(client.birth_date)} años` : null} />
          ...
          {preferences.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-2">
              {preferences.map(([key, val]) => (
                <span key={key} className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-medium text-indigo-700">
